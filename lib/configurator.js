    ...
    addSubAppDefaults(config) {
        ...
        if (!config.moduleDir) {
            ...
            if (!config.hasOwnProperty('moduleid')) {
                throw new Error('You need to specify "config.moduleDir". Else set "config.moduleid" so we can guess.');
            }

            config.moduleDir = join(config.basepath, config.moduleid);

            this.logger.warn('You did not specify a "moduleDir" for %s?', config.moduleid);
