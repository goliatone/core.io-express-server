...
class Configurator {
    ...
    addDefaults(config) {
        ...
        if (!config.basepath) {
            ...
            config.basepath = './modules/' + config.moduleid;

            this.logger.warn('You did not specify a "basepath" for %s?', config.moduleid);
            this.logger.warn('We are making a guess: %s', config.basepath);
        } else {
            this.logger.warn('We will be loading views and assets for %s from "%s"',config.moduleid, config.basepath);
        }
        ...
        if (!config.routes) {
            config.routes = {
                path: [join(config.basepath, 'routes')]
            };
        }
