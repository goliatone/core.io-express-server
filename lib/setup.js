...
class Setup {
    ...
    addMiddleware() {
        ...
        let publicpath = this.app.modulePublic;
        let userPublic = path.join(config.basepath, config.moduleid, 'public');
        let userPublicExists = fs.existsSync(userPublic);

        if (config.publicPath && fs.existsSync(config.publicPath)) { 
            publicpath = config.publicPath;
        } else if(userPublicExists) {
            publicpath = userPublic;
        }

        config.publicPath = publicpath;

        /**
         * TODO: check all submodules, see if they have 
         * a public folder.
        ...
        const defaultFavicon = path.join(this.app.modulePublic, 'favicon.ico');
        const userFavicon = path.join(userPublic, 'favicon.ico');

        let favipath = defaultFavicon;
        
        if (config.faviconPath) { 
            favipath = config.faviconPath;
        } else if(userPublicExists) { 
            favipath = userFavicon;
        }

        if(!fs.existsSync(favipath) && config.doNotUseDefaultFavicon) {
            return;
        } else {
            favipath = defaultFavicon;
        }
