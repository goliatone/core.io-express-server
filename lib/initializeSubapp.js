...
module.exports = function $initializeSubAppWrapper(App, subappId) {
    ...
    return function $initializeSubApp(context, config) {
        ...
        logger.info('-> %s config %s', subappId, Object.keys(config));
        logger.info(config.moduleid, config.basepath, config.mount);
        
        context.resolve(config.dependencies, true).then(() => {
            ...
            context.on('server.pre', (configurator) => {
                 ...
                 */
                configurator.addSubAppDefaults(config);

                logger.info(
                    '%s mounting application. Mount path: %s',
                    subappId,
                    config.mount
                );
                ...
                try {
                    ...
                    logger.error(
                        'Error initializing sub application %s.',
                        subappId
                    );
                    throw err;
                }
